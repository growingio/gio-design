...
function merge(selected: CheckboxValueType[], option: CheckboxOptionType, registeredValues: CheckboxValueType[]): CheckboxValueType[] {
...
const CheckboxGroup: React.FC<CheckboxGroupProps> = ({
  ...
  const toggleOption = React.useCallback((option: CheckboxOptionType) => {
    if (value === undefined) {
  ...
  if (options.length > 0) {
    customChildren = options.map(option => {

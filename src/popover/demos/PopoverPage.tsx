...
export default function ListPage() {
  ...
  return (
    <>
      ...
      <Subheading>{formatMessage({ defaultMessage: '弹出框箭头' })}</Subheading>
      ...
      <Canvas>
        <Story id="upgraded-popover--arrow" />
      </Canvas>
      ...
      <Subheading>{formatMessage({ defaultMessage: '自带组件Ref触发器' })}</Subheading>
      <Description>
      ...
      <Subheading>{formatMessage({ defaultMessage: '字符串触发器' })}</Subheading>
      <Description>
      ...
      <Description>
        ...
        来设定弹出框的挂载点，可以指定某个div或者body上。如下面的案例，左边container设置在最外层，右边的container设置在内层，并设置30%的透明度。可以看到弹出框的透明度也发生了变化，而左边的弹出框无变化。`}
      </Description>
      <Canvas>
      ...
      <Subheading>{formatMessage({ defaultMessage: 'Delay和hideDelay' })}</Subheading>
      <Description>

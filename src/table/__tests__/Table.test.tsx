...
describe('Testing Table', () => {
  ...
  it('custom empty', () => {
    const { container } = render(
      <Table
        ...
        dataSource={[]}
        empty={{ type: 'empty-result', description: '无结果' }}
      />
    );
  ...
  it('render title and footer', () => {
    ...
    const { container } = render(
      <Table
        ...
        title={() => <h2 className="my-table-title">名单</h2>}
  ...
  it('render row index', () => {
    ...
    const { container } = render(
      <Table rowKey="id" showIndex columns={columns} dataSource={data} pagination={false} />
    );
    expect(
      Array.from(container.querySelectorAll('.gio-table-tbody td:first-child')).map((e) => e.textContent)
    ).toStrictEqual(['1', '2']);
  });
  ...
  it('onHackRow', () => {
    ...
    const { container } = render(
      <Table hackRowEvent rowKey="id" columns={columns} dataSource={data} pagination={false} />
    );
    fireEvent.click(container.querySelector('.gio-table-tbody td:first-child'));
  });
  ...
  it('onHackRow and onRow', () => {
    ...
    const { container } = render(
      <Table
        hackRowEvent
        onRow={(r, _index) => ({
          onClick: handleRowClick(r, _index),
  ...
  it('table <Title> component', () => {
    ...
    expect(
      container.querySelector('.gio-table-thead th:first-child .gio-table-column-title-text')
    ).toBeEmptyDOMElement();
  });
  it('table rowClassName', () => {
    ...
    const { container } = render(
      <Table
        ...
        dataSource={data}
        showHover={false}
        rowClassName={(record) => (record.id === 2 ? 'jim' : '')}
      />
    );

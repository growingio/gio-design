...
describe('Testing Table rowSelection', () => {
  ...
  function createTable(tableProps: TableProps<DataType> = {}, columns = [column]) {
    return <Table<DataType> columns={columns} dataSource={data} pagination={false} rowSelection={{}} {...tableProps} />;
  }
  it('select default row', () => {
    ...
    expect(container.querySelectorAll('.gio-table-tbody td:nth-child(1) input[type="checkbox"]')[0]).toHaveAttribute(
      'checked'
    );
  });

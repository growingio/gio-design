...
describe('Testing Table rowSelection', () => {
  ...
  test('getRowKey function', () => {
    ...
    // @ts-ignore
    const rowKeyNumber = getRowKey(dataSource[0], 123);
    expect(rowKeyNumber).toBe('1');
  });

  test('rowSelection all props', () => {
    const changeMock = jest.fn();
    const { result } = renderHook(() =>
      useSelection(
        dataSource,
        { onChange: changeMock, selectedRowKeys: ['胡彦斌'], columnWidth: 60, fixed: 'right' },
        { rowKey: 'name' }
      )
    );
  ...
  test('rowSelection without onChange prop', async () => {
    const { container } = render(
      <Table dataSource={dataSource} columns={columns} pagination={false} rowSelection={{ selectedRowKeys: [] }} />
    );
    expect(container.getElementsByClassName('gio-checkbox-checked')).toHaveLength(0);
    fireEvent.click(container.getElementsByClassName('gio-checkbox-input')[0]);
    fireEvent.click(container.getElementsByClassName('gio-checkbox-input')[1]);

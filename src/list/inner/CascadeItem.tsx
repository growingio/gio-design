...
import { CascadeItemProps, OptionProps, BaseItemProps } from '../interface';
...
const CascadeItem: React.ForwardRefRenderFunction<
  HTMLLIElement,
  CascadeItemProps & Omit<DOMAttributes<HTMLLIElement>, 'onClick'>
> & { isItem?: boolean } = (
          ...
          <List className={`${prefixCls}--list`}>
            {childrens.map((child) => (
              <CascadeItem
                ...
                label={child.label}
                value={child.value}
                items={child.items as CascadeItemProps[]}
              />
            ))}
          </List>
        </ListContext.Provider>
...
CascadeItem.isItem = true;
export default WithRef(CascadeItem);
